...
public class IndexWriterPool {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(IndexWriterPool.class);
	...
	private IndexWriter createIndexWriter(String indexDirName) {
		try {
			File indexPath = LuceneUtil.getFilePath(indexRootDirectory,
					indexDirName);
			IndexWriterConfig conf = new IndexWriterConfig(
					LuceneUtil.CURRENT_VERSION, new PaodingAnalyzer());
	 ...
	 */
	public void commit() {
		logger.info("begin to commit all IndexWiters of pool,the pool size:{}",
				size());
		synchronized (indexWriterMap) {
			Iterator<Entry<String, IndexWriter>> iterator = indexWriterMap
					.entrySet().iterator();
			while (iterator.hasNext()) {
				...
				try {
					...
					try {
						...
						logger.error(
								"IOException while closing indexWriter,the root cause:{}",
								ex.getMessage());
					}
				}
			}
		...
		synchronized (indexWriterMap) {
			...
			while (iterator.hasNext()) {
				...
				try {
					...
					logger.error(
							"Exception while commiting {},the root cause:{}",
							entry.getKey(), e.getMessage());
				} finally {
					try {
						indexWriter.close();
			...
			for (String indexDirName : indexDirNameList) {
				indexWriterMap.put(indexDirName,
						createIndexWriter(indexDirName));
			}
		}
		...
		synchronized (indexWriterMap) {
			...
			while (iterator.hasNext()) {
				...
				try {
					...
					logger.error(
							"Exception while closing {},the root cause:{}",
							entry.getKey(), e.getMessage());
					try {
						indexWriter.close();
						iterator.remove();
	...
	public void optimize() {
		logger.info("begin to optimize at {}",
				DateUtil.date2String(new Date(), DATE_FORMATTER));
		synchronized (indexWriterMap) {
			Iterator<Entry<String, IndexWriter>> iterator = indexWriterMap
					.entrySet().iterator();
			while (iterator.hasNext()) {
				...
				try {
					...
					logger.error(
							"Exception while optimizing {},the root cause:{}",
							entry.getKey(), e.getMessage());
					try {
						indexWriter.close();
						iterator.remove();
