package cn.lichengwu.utils.security;
...
public final class MD5 {
    ...
    /*
     * getMD5ofStr是类MD5最主要的公共方法，入口参数是你想要进行MD5变换的字符串 返回的是变换完的结果．
     */
    ...
    private static void md5Init(long[] count, long[] state) {
        count[0] = 0L;
        count[1] = 0L;
     ...
     */
    private static void md5Update(byte[] inbuf, int inputLen, long[] count, byte[] buffer,
            long[] state) {
        int i, index, partLen;
     ...
     * md5Final整理和填写输出结果
     */
    private static void md5Final(long[] count, byte[] digest, long[] state, byte[] buffer) {
        ...
        md5Update(PADDING, padLen, count, buffer, state);
        // /* Append length (before padding) */
        md5Update(bits, 8, count, buffer, state);
     ...
     */
    private static void md5Transform(byte block[], long[] state) {
